...
// uEmuera: copy bundled game to Documents on first launch
static void CopyBundledGameIfNeeded()
{
    @autoreleasepool
    {
        NSString* bundlePath = [NSString stringWithUTF8String: UnityDataBundleDir()];
        NSString* source = [bundlePath stringByAppendingPathComponent:@"Data/Raw/eratoho"];
        NSString* documents = [NSString stringWithUTF8String: UnityDocumentsDir()];
        NSString* dest = [documents stringByAppendingPathComponent:@"eratoho"];
        ...
        if (![fm fileExistsAtPath:source isDirectory:&isDir] || !isDir)
        {
            NSLog(@"[uEmuera] Bundled game not found at %@", source);
            return;
        }
        ...
        if (err != nil)
        {
            NSLog(@"[uEmuera] Failed to create dest %@: %@", dest, err);
            return;
        }

        NSArray* items = [fm contentsOfDirectoryAtPath:source error:&err];
        if (err != nil)
        {
            NSLog(@"[uEmuera] Failed to list source %@: %@", source, err);
            return;
        }

        for (NSString* item in items)
        {
            NSString* srcItem = [source stringByAppendingPathComponent:item];
            NSString* dstItem = [dest stringByAppendingPathComponent:item];
            if ([fm fileExistsAtPath:dstItem])
                continue;
            err = nil;
            [fm copyItemAtPath:srcItem toPath:dstItem error:&err];
            if (err != nil)
                NSLog(@"[uEmuera] Failed to copy %@: %@", item, err);
        }
    }
}
